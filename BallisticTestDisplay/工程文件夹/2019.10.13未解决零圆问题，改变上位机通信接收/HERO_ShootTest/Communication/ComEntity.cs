...
namespace Communication
{
    ...
    public class ComEntity
    {
        ...
        public void CloseSerialPort()
        {
            try
            {
                //先清空标志，再关闭串口，防止线程对组件的访问引发异常
                Series.isopen = false;

                sp.Close();

                //终止线程（因为感觉不必终止线程，所以注释掉了
                //再有是因为要在线程中关闭串口，所以为了避免关闭失败）
                //th.Abort();
            }
            catch (System.Exception ex)
            {
        ...
        void SpReceiveMsg()
        {
            while (true)
            {
                ...
                {
                    SaveInfo("<串口接收线程>:" + ex.Message + "串口已关闭。" + "\r\n");
                    CloseSerialPort();
                }
        ...
        protected virtual byte[] DealDataToSend(byte SendID) { return null; }
        ...
        public void SendData(byte SendID)
        {
            byte[] DataToSend = DealDataToSend(SendID);
